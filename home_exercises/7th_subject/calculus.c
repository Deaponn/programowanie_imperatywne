...
double fast_power(double number, int power) {
    if (power == 0) return 1;
    if (power == 1) return number;
    if (power % 2 == 0) return fast_power(number, power / 2) * fast_power(number, power / 2);
    return number * fast_power(number, power >> 1) * fast_power(number, power >> 1);
}
...
typedef double (*Func1vFp)(double x);  // Definiowanie nazwy dla typu wskaznika do funkcji jedej zmiennej 

double f_poly(double x) {  //    polynomial  a[0]+a[1]x+ ... + a[n]x^n
    double weights[] = {2, -4, 0, 3.5, 1.35, -6.25};
    double result = 0;
    for (int i = 0; i < 6; i++) {
        result += weights[i] * fast_power(x, 5 - i);
    }
    return result;
}
...
double quad_trap(Func1vFp func, double a, double b, int n) {  // Trapezow
    ...
    double curr_arg = a;
    double prev_value;
    double curr_value = (*func)(curr_arg);
    double area = 0;
    for (int i = 0; i < n; i++) {
        prev_value = curr_value;
        curr_arg += length;
        curr_value = (*func)(curr_arg);
        area += length / 2 * (prev_value + curr_value);
    }
    return area;
}
...
double quad_simpson(Func1vFp f, double a, double b, int n) {  // Simpsona
    double length = (b - a) / n;
    double curr_arg = a;
    double start = (*f)(curr_arg);
    double middle = (*f)(curr_arg + length / 2);
    curr_arg += length;
    double end = (*f)(curr_arg);
    double area = (length) / 6 * (start + 4 * middle + end);
    for (int i = 0; i < n - 1; i++) {
        start = end;
        middle = (*f)(curr_arg + length / 2);
        curr_arg += length;
        end = (*f)(curr_arg);
        area += (length) / 6 * (start + 4 * middle + end);
    }
    return area;
}

// Definiowanie nazwy dla typu wskaznika do funkcji obliczającej kwadraturę funkcji jednej zmiennej  
typedef double (*QuadratureFp)(Func1vFp func, double a, double b, int n);  
...
typedef double Func2vFp(double x, double y);  // Definiowanie nazwy dla typu wskaznika do funkcji dwoch zmiennych
