...
void aver_varian(const double tab[], size_t n, double *arith_average, double *variance) {
    ...
    double variant;
    for (int i = 0; i < n; i++) {
        double summand = tab[i] - average;
        variant += summand * summand;
    }
    variant /= n;
...
int random_result(double probability) {
    int random_number = rand();
    return random_number < probability * RAND_MAX;
}
...
void two_dice_probab_distrib(double distrib[], int throws_num) {
    ...
    for (int i = 0; i < throws_num; i++) {
        roll1 = rand_from_interval(0, 5);
        roll2 = rand_from_interval(0, 5);
        distrib[roll1 + roll2]++;
    }
    for (int i = 0; i < 12; i++) {
...
void print_histogram(const double tab[], size_t n, int x_start, double y_scale, char mark) {
    int longest_x = log10((n - 1) + x_start);
    for (int i = 0; i < n; i++) {
        int current_x_length = log10(i + x_start);
        for (int j = 0; j < longest_x - current_x_length; j++) {
            printf(" ");
        }
        printf("%d |", i + x_start);
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) {
    for (int i = 0; i < n; i++) {
        int winner_door = rand_from_interval(0, 2);
        int choice = rand_from_interval(0, 2);
        if (winner_door == choice) *p_nonswitch_wins = *p_nonswitch_wins + 1;
        else *p_switch_wins = *p_switch_wins + 1;
    }
}
